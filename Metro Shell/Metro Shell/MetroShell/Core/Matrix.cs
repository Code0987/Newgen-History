...
namespace Ftware.Apps.MetroShell.Core
{
    public class MetroShellMatrix
    {
        ...
        public MetroShellMatrix(int c, int r)
        {
        ...
        public MetroShellCell GetFreeCell(int length)
        {
            for (var i = 0; i < maxCols; i++) //looks up in columns
            {
                ...
                if (index != -1)
                {
                    var cell = new MetroShellCell(i, index);
                    return cell;
                }
            }
            return new MetroShellCell(-1, -1);
        }
