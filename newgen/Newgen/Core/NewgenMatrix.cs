...
namespace Newgen.Core
{
    public class NewgenMatrix
    {
        ...
        public NewgenMatrix(int c, int r)
        {
        ...
        public NewgenCell GetFreeCell(int length)
        {
            for (var i = 0; i < maxCols; i++) //looks up in columns
            {
                ...
                if (index != -1)
                {
                    var cell = new NewgenCell(i, index);
                    return cell;
                }
            }
            return new NewgenCell(-1, -1);
        }
